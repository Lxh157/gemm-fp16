// src/fp16_mma/gemm_mma_fp16acc_m16n64_staged_cpasync_k64_4x1_skew16.cu
#include "utils.cuh"
...
// 4x1 warps/block. Each warp computes one 16x64 tile.
// CTA output tile: 64x64. Shared stage depth: K=64.
// Global-to-shared staging uses double-buffered cp.async.
constexpr int WARPS_PER_BLOCK_M = 4;
constexpr int WARPS_PER_BLOCK_N = 1;
...
constexpr int B_NUM_CHUNKS = BLOCK_K * B_ROW_CHUNKS; // 512
...
__global__ void gemm_mma_fp16acc_m16n64_staged_cpasync_k64_4x1_skew16_kernel(
    const half* __restrict__ A,
    const half* __restrict__ B,
    float* __restrict__ C,
    int M, int N, int K) {

    __shared__ half smemA[2][BLOCK_M * SMEM_STRIDE_A]; // [2][64][80]
    __shared__ half smemB[2][BLOCK_K * SMEM_STRIDE_B]; // [2][64][80]
...
void launch_gemm_mma_fp16acc_m16n64_staged_cpasync_k64_4x1_skew16(
    const half* dA, const half* dB, float* dC,
    int M, int N, int K, cudaStream_t stream) {

    if (M % BLOCK_M != 0 || N % BLOCK_N != 0 || K % BLOCK_K != 0) {
        std::fprintf(stderr,
                     "gemm_mma_fp16acc_m16n64_staged_cpasync_k64_4x1_skew16: currently requires "
                     "M %% 64 == 0, N %% 64 == 0, and K %% 64 == 0. "
                     "Got M=%d N=%d K=%d\n",
                     M, N, K);
    ...
    gemm_mma_fp16acc_m16n64_staged_cpasync_k64_4x1_skew16_kernel<<<grid, block, 0, stream>>>(
        dA, dB, dC, M, N, K);
