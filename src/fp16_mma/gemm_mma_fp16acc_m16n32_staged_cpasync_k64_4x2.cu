// src/fp16_mma/gemm_mma_fp16acc_m16n32_staged_cpasync_k64_4x2.cu
#include "utils.cuh"
...
__global__ void gemm_mma_fp16acc_m16n32_staged_cpasync_k64_4x2_kernel(
    const half* __restrict__ A,
    const half* __restrict__ B,
    float* __restrict__ C,
    int M, int N, int K) {

    __shared__ half smemA[2][BLOCK_M * SMEM_STRIDE_A]; // [2][64][72]
    __shared__ half smemB[2][BLOCK_K * SMEM_STRIDE_B]; // [2][64][72]
...
void launch_gemm_mma_fp16acc_m16n32_staged_cpasync_k64_4x2(
    const half* dA, const half* dB, float* dC,
    int M, int N, int K, cudaStream_t stream) {

    if (M % BLOCK_M != 0 || N % BLOCK_N != 0 || K % BLOCK_K != 0) {
        std::fprintf(stderr,
                     "gemm_mma_fp16acc_m16n32_staged_cpasync_k64_4x2: currently requires "
    ...
    gemm_mma_fp16acc_m16n32_staged_cpasync_k64_4x2_kernel<<<grid, block, 0, stream>>>(
        dA, dB, dC, M, N, K);
