// src/gemm_mma_fp16acc_m16n32.cu
#include "utils.cuh"
...
__global__ void gemm_mma_fp16acc_m16n32_kernel(const half* __restrict__ A,
                                               const half* __restrict__ B,
                                               float* __restrict__ C,
                                               int M, int N, int K) {
    __shared__ half smemA[BLOCK_M * SMEM_STRIDE_A]; // [64][24]
...
void launch_gemm_mma_fp16acc_m16n32(const half* dA, const half* dB, float* dC,
    ...
    if (K % MMA_K != 0) {
        std::fprintf(stderr,
                     "gemm_mma_fp16acc_m16n32: currently requires K %% 16 == 0. "
                     "Got K=%d\n",
                     K);
    ...
    gemm_mma_fp16acc_m16n32_kernel<<<grid, block, 0, stream>>>(dA, dB, dC, M, N, K);
