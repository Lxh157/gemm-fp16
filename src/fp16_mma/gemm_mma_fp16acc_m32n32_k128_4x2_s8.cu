// src/fp16_mma/gemm_mma_fp16acc_m32n32_k128_4x2_s8.cu
#include "utils.cuh"
...
constexpr int BLOCK_K = 8 * MMA_K;                       // 128

constexpr int SKEW_HALF = 8;
constexpr int SMEM_STRIDE_A = BLOCK_K + SKEW_HALF; // 136
...
constexpr int A_ROW_CHUNKS = BLOCK_K / CHUNK_HALF;   // 16
constexpr int A_NUM_CHUNKS = BLOCK_M * A_ROW_CHUNKS; // 2048
...
__global__ void gemm_mma_fp16acc_m32n32_k128_4x2_s8_kernel(
    const half* __restrict__ A,
    const half* __restrict__ B,
    ...
    for (int k0 = 0; k0 < K; k0 += BLOCK_K) {
        ...
        for (int kk = 0; kk < BLOCK_K; kk += MMA_K) {
            mma_32x32_step(
                smemA[read_buf], smemB[read_buf], warp_m, warp_n, lane, kk,
                c00, c01, c02, c03,
                c10, c11, c12, c13,
                c20, c21, c22, c23,
                c30, c31, c32, c33,
...
void launch_gemm_mma_fp16acc_m32n32_k128_4x2_s8(
    const half* dA, const half* dB, float* dC,
    int M, int N, int K, cudaStream_t stream) {

    if (M % BLOCK_M != 0 || N % BLOCK_N != 0 || K % BLOCK_K != 0) {
        std::fprintf(stderr,
                     "gemm_mma_fp16acc_m32n32_k128_4x2_s8: currently requires "
                     "M %% 128 == 0, N %% 64 == 0, and K %% 128 == 0. "
                     "Got M=%d N=%d K=%d\n",
                     M, N, K);
    ...
    CHECK_CUDA(cudaFuncSetAttribute(
        gemm_mma_fp16acc_m32n32_k128_4x2_s8_kernel,
        cudaFuncAttributeMaxDynamicSharedMemorySize,
        SMEM_TOTAL_BYTES));

    gemm_mma_fp16acc_m32n32_k128_4x2_s8_kernel<<<grid, block, SMEM_TOTAL_BYTES, stream>>>(
        dA, dB, dC, M, N, K);
