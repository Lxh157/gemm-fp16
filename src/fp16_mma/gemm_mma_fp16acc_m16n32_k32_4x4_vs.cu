// K32 4x4-warp CTA with vectorized C stores and no final CTA sync.
#include "utils.cuh"
...
__global__ void gemm_mma_fp16acc_m16n32_k32_4x4_vs_kernel(
    const half* __restrict__ A,
    const half* __restrict__ B,
    float* __restrict__ C,
    int M, int N, int K) {

    __shared__ half smemA[2][BLOCK_M * SMEM_STRIDE_A]; // [2][64][40]
    __shared__ half smemB[2][BLOCK_K * SMEM_STRIDE_B]; // [2][32][136]
...
void launch_gemm_mma_fp16acc_m16n32_k32_4x4_vs(
    const half* dA, const half* dB, float* dC,
    int M, int N, int K, cudaStream_t stream) {

    if (M % BLOCK_M != 0 || N % BLOCK_N != 0 || K % BLOCK_K != 0) {
        std::fprintf(stderr,
                     "gemm_mma_fp16acc_m16n32_k32_4x4_vs: currently requires "
                     "M %% 64 == 0, N %% 128 == 0, and K %% 32 == 0. "
                     "Got M=%d N=%d K=%d\n",
                     M, N, K);
    ...
    gemm_mma_fp16acc_m16n32_k32_4x4_vs_kernel<<<grid, block, 0, stream>>>(
        dA, dB, dC, M, N, K);
