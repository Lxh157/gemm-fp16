...
__global__ void gemm_mma_fp16acc_m16n32_k32_s16_vs_kernel(
    const half* __restrict__ A,
    const half* __restrict__ B,
...
void launch_gemm_mma_fp16acc_m16n32_k32_s16_vs(
    const half* dA, const half* dB, float* dC,
    int M, int N, int K, cudaStream_t stream) {

    if (M % BLOCK_M != 0 || N % BLOCK_N != 0 || K % BLOCK_K != 0) {
        std::fprintf(stderr,
                     "gemm_mma_fp16acc_m16n32_k32_s16_vs: currently requires "
    ...
    gemm_mma_fp16acc_m16n32_k32_s16_vs_kernel<<<grid, block, 0, stream>>>(
        dA, dB, dC, M, N, K);
