// src/fp16_mma/gemm_mma_fp16acc_m32n32_k64_4x2.cu
#include "utils.cuh"
...
__global__ void gemm_mma_fp16acc_m32n32_k64_4x2_kernel(
    const half* __restrict__ A,
    const half* __restrict__ B,
...
void launch_gemm_mma_fp16acc_m32n32_k64_4x2(
    const half* dA, const half* dB, float* dC,
    int M, int N, int K, cudaStream_t stream) {

    if (M % BLOCK_M != 0 || N % BLOCK_N != 0 || K % BLOCK_K != 0) {
        std::fprintf(stderr,
                     "gemm_mma_fp16acc_m32n32_k64_4x2: currently requires "
    ...
    CHECK_CUDA(cudaFuncSetAttribute(
        gemm_mma_fp16acc_m32n32_k64_4x2_kernel,
        cudaFuncAttributeMaxDynamicSharedMemorySize,
        SMEM_TOTAL_BYTES));

    gemm_mma_fp16acc_m32n32_k64_4x2_kernel<<<grid, block, SMEM_TOTAL_BYTES, stream>>>(
        dA, dB, dC, M, N, K);
