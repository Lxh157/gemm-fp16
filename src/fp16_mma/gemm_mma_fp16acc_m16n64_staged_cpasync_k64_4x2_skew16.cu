// src/fp16_mma/gemm_mma_fp16acc_m16n64_staged_cpasync_k64_4x2_skew16.cu
#include "utils.cuh"
...
// 4x2 warps/block. Each warp computes one 16x64 tile.
// CTA output tile: 64x128. Shared stage depth: K=64.
...
constexpr int WARP_TILE_N = 8 * MMA_N;                   // 64
constexpr int BLOCK_M = WARPS_PER_BLOCK_M * WARP_TILE_M; // 64
constexpr int BLOCK_N = WARPS_PER_BLOCK_N * WARP_TILE_N; // 128
constexpr int BLOCK_K = 4 * MMA_K;                       // 64

constexpr int SKEW_HALF = 16;
constexpr int SMEM_STRIDE_A = BLOCK_K + SKEW_HALF; // 80
constexpr int SMEM_STRIDE_B = BLOCK_N + SKEW_HALF; // 144

constexpr int CHUNK_BYTES = 16;
constexpr int CHUNK_HALF = CHUNK_BYTES / sizeof(half); // 8 half

constexpr int A_ROW_CHUNKS = BLOCK_K / CHUNK_HALF;   // 8
constexpr int A_NUM_CHUNKS = BLOCK_M * A_ROW_CHUNKS; // 512

constexpr int B_ROW_CHUNKS = BLOCK_N / CHUNK_HALF;    // 16
constexpr int B_NUM_CHUNKS = BLOCK_K * B_ROW_CHUNKS;  // 1024

constexpr int SMEM_A_STAGE_ELEMS = BLOCK_M * SMEM_STRIDE_A;
constexpr int SMEM_B_STAGE_ELEMS = BLOCK_K * SMEM_STRIDE_B;
constexpr int SMEM_TOTAL_BYTES =
    2 * (SMEM_A_STAGE_ELEMS + SMEM_B_STAGE_ELEMS) * sizeof(half);
...
__device__ __forceinline__ void mma_16x64_step(
    ...
    float& c40, float& c41, float& c42, float& c43,
    float& c50, float& c51, float& c52, float& c53,
    float& c60, float& c61, float& c62, float& c63,
    ...
    uint32_t b_addr4 = cvta_to_shared_u32(
        &smemB[(kk + (lane % 16)) * SMEM_STRIDE_B + b_col_base + 32]);
    ldmatrix_x2_trans(b0, b1, b_addr4);
    mma_m16n8k16(a0, a1, a2, a3, b0, b1, c40, c41, c42, c43);

    uint32_t b_addr5 = cvta_to_shared_u32(
        &smemB[(kk + (lane % 16)) * SMEM_STRIDE_B + b_col_base + 40]);
    ldmatrix_x2_trans(b0, b1, b_addr5);
    mma_m16n8k16(a0, a1, a2, a3, b0, b1, c50, c51, c52, c53);

    uint32_t b_addr6 = cvta_to_shared_u32(
        &smemB[(kk + (lane % 16)) * SMEM_STRIDE_B + b_col_base + 48]);
    ldmatrix_x2_trans(b0, b1, b_addr6);
    mma_m16n8k16(a0, a1, a2, a3, b0, b1, c60, c61, c62, c63);

    uint32_t b_addr7 = cvta_to_shared_u32(
        &smemB[(kk + (lane % 16)) * SMEM_STRIDE_B + b_col_base + 56]);
    ldmatrix_x2_trans(b0, b1, b_addr7);
    mma_m16n8k16(a0, a1, a2, a3, b0, b1, c70, c71, c72, c73);
}
...
__global__ void gemm_mma_fp16acc_m16n64_staged_cpasync_k64_4x2_skew16_kernel(
    const half* __restrict__ A,
    const half* __restrict__ B,
    ...
    half* smemA[2] = {smem, smem + SMEM_A_STAGE_ELEMS};
    half* smemB[2] = {
        smem + 2 * SMEM_A_STAGE_ELEMS,
        smem + 2 * SMEM_A_STAGE_ELEMS + SMEM_B_STAGE_ELEMS,
    };
    ...
    float c40 = 0.f, c41 = 0.f, c42 = 0.f, c43 = 0.f;
    float c50 = 0.f, c51 = 0.f, c52 = 0.f, c53 = 0.f;
    float c60 = 0.f, c61 = 0.f, c62 = 0.f, c63 = 0.f;
    float c70 = 0.f, c71 = 0.f, c72 = 0.f, c73 = 0.f;
    ...
    for (int k0 = 0; k0 < K; k0 += BLOCK_K) {
        ...
        mma_16x64_step(
            smemA[read_buf], smemB[read_buf], warp_m, warp_n, lane, 2 * MMA_K,
            c00, c01, c02, c03,
            c10, c11, c12, c13,
            c20, c21, c22, c23,
            c30, c31, c32, c33,
        ...
        mma_16x64_step(
            smemA[read_buf], smemB[read_buf], warp_m, warp_n, lane, 3 * MMA_K,
            c00, c01, c02, c03,
            c10, c11, c12, c13,
            c20, c21, c22, c23,
            c30, c31, c32, c33,
    ...
    store_c_frag_16x8(C, N, lane, tile_row, tile_col + 24,
                      c30, c31, c32, c33, M, N);
    store_c_frag_16x8(C, N, lane, tile_row, tile_col + 32,
                      c40, c41, c42, c43, M, N);
    store_c_frag_16x8(C, N, lane, tile_row, tile_col + 40,
                      c50, c51, c52, c53, M, N);
    store_c_frag_16x8(C, N, lane, tile_row, tile_col + 48,
                      c60, c61, c62, c63, M, N);
    store_c_frag_16x8(C, N, lane, tile_row, tile_col + 56,
                      c70, c71, c72, c73, M, N);
}

} // namespace

void launch_gemm_mma_fp16acc_m16n64_staged_cpasync_k64_4x2_skew16(
    const half* dA, const half* dB, float* dC,
    int M, int N, int K, cudaStream_t stream) {

    if (M % BLOCK_M != 0 || N % BLOCK_N != 0 || K % BLOCK_K != 0) {
        std::fprintf(stderr,
                     "gemm_mma_fp16acc_m16n64_staged_cpasync_k64_4x2_skew16: currently requires "
                     "M %% 64 == 0, N %% 128 == 0, and K %% 64 == 0. "
                     "Got M=%d N=%d K=%d\n",
                     M, N, K);
    ...
    CHECK_CUDA(cudaFuncSetAttribute(
        gemm_mma_fp16acc_m16n64_staged_cpasync_k64_4x2_skew16_kernel,
        cudaFuncAttributeMaxDynamicSharedMemorySize,
        SMEM_TOTAL_BYTES));

    gemm_mma_fp16acc_m16n64_staged_cpasync_k64_4x2_skew16_kernel<<<grid, block, SMEM_TOTAL_BYTES, stream>>>(
        dA, dB, dC, M, N, K);
