// src/fp16_mma/gemm_mma_fp16acc_m32n32_k64_4x2_s8.cu
#include "utils.cuh"
...
// 4x2 warps/block. Each warp computes one 32x32 tile.
// CTA output tile: 128x64. Shared stage depth: K=64.
...
constexpr int A_NUM_CHUNKS = BLOCK_M * A_ROW_CHUNKS; // 1024
...
__device__ __forceinline__ void mma_32x32_step(
    ...
    uint32_t a4, a5, a6, a7;
    {
        ...
        uint32_t addr0 = cvta_to_shared_u32(
            &smemA[ldm_row * SMEM_STRIDE_A + ldm_col]);
        uint32_t addr1 = cvta_to_shared_u32(
            &smemA[(ldm_row + MMA_M) * SMEM_STRIDE_A + ldm_col]);
    ...
    mma_m16n8k16(a4, a5, a6, a7, b0, b1, c40, c41, c42, c43);
    ...
    mma_m16n8k16(a4, a5, a6, a7, b0, b1, c50, c51, c52, c53);
    ...
    mma_m16n8k16(a4, a5, a6, a7, b0, b1, c60, c61, c62, c63);
    ...
    mma_m16n8k16(a4, a5, a6, a7, b0, b1, c70, c71, c72, c73);
}
...
__global__ void gemm_mma_fp16acc_m32n32_k64_4x2_s8_kernel(
    const half* __restrict__ A,
    const half* __restrict__ B,
    ...
    store_c_frag_16x8(C, N, lane, tile_row, tile_col + 24,
                      c30, c31, c32, c33, M, N);
    store_c_frag_16x8(C, N, lane, tile_row + MMA_M, tile_col + 0,
                      c40, c41, c42, c43, M, N);
    store_c_frag_16x8(C, N, lane, tile_row + MMA_M, tile_col + 8,
                      c50, c51, c52, c53, M, N);
    store_c_frag_16x8(C, N, lane, tile_row + MMA_M, tile_col + 16,
                      c60, c61, c62, c63, M, N);
    store_c_frag_16x8(C, N, lane, tile_row + MMA_M, tile_col + 24,
                      c70, c71, c72, c73, M, N);
}

} // namespace

void launch_gemm_mma_fp16acc_m32n32_k64_4x2_s8(
    const half* dA, const half* dB, float* dC,
    int M, int N, int K, cudaStream_t stream) {

    if (M % BLOCK_M != 0 || N % BLOCK_N != 0 || K % BLOCK_K != 0) {
        std::fprintf(stderr,
                     "gemm_mma_fp16acc_m32n32_k64_4x2_s8: currently requires "
                     "M %% 128 == 0, N %% 64 == 0, and K %% 64 == 0. "
                     "Got M=%d N=%d K=%d\n",
                     M, N, K);
    ...
    CHECK_CUDA(cudaFuncSetAttribute(
        gemm_mma_fp16acc_m32n32_k64_4x2_s8_kernel,
        cudaFuncAttributeMaxDynamicSharedMemorySize,
        SMEM_TOTAL_BYTES));

    gemm_mma_fp16acc_m32n32_k64_4x2_s8_kernel<<<grid, block, SMEM_TOTAL_BYTES, stream>>>(
        dA, dB, dC, M, N, K);
