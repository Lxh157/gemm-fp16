// src/fp16_mma/gemm_mma_fp16acc_m16n32_staged_cpasync_ca_k64_4x2_skew16.cu
#include "utils.cuh"
...
__device__ __forceinline__ void cp_async_ca_16B(void* smem_ptr,
                                                const void* gmem_ptr) {
...
__device__ __forceinline__ void load_stage_to_shared_cpasync(
    ...
    for (int chunk = tid; chunk < A_NUM_CHUNKS; chunk += THREADS_PER_BLOCK) {
        ...
        cp_async_ca_16B(smem_dst, gmem_src);
    }
...
__global__ void gemm_mma_fp16acc_m16n32_staged_cpasync_ca_k64_4x2_skew16_kernel(
    const half* __restrict__ A,
    const half* __restrict__ B,
...
void launch_gemm_mma_fp16acc_m16n32_staged_cpasync_ca_k64_4x2_skew16(
    const half* dA, const half* dB, float* dC,
    int M, int N, int K, cudaStream_t stream) {

    if (M % BLOCK_M != 0 || N % BLOCK_N != 0 || K % BLOCK_K != 0) {
        std::fprintf(stderr,
                     "gemm_mma_fp16acc_m16n32_staged_cpasync_ca_k64_4x2_skew16: currently requires "
    ...
    gemm_mma_fp16acc_m16n32_staged_cpasync_ca_k64_4x2_skew16_kernel<<<grid, block, 0, stream>>>(
        dA, dB, dC, M, N, K);
