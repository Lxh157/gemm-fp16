...
constexpr int SKEW_A_HALF = 8;
constexpr int SKEW_B_HALF = 16;
constexpr int SMEM_STRIDE_A = BLOCK_K + SKEW_A_HALF; // 40
constexpr int SMEM_STRIDE_B = BLOCK_N + SKEW_B_HALF; // 80
...
__global__ void gemm_mma_fp16acc_m16n32_k32_b16_vs_kernel(
    const half* __restrict__ A,
    const half* __restrict__ B,
    ...
    __shared__ half smemB[2][BLOCK_K * SMEM_STRIDE_B]; // [2][32][80]
...
void launch_gemm_mma_fp16acc_m16n32_k32_b16_vs(
    const half* dA, const half* dB, float* dC,
    int M, int N, int K, cudaStream_t stream) {

    if (M % BLOCK_M != 0 || N % BLOCK_N != 0 || K % BLOCK_K != 0) {
        std::fprintf(stderr,
                     "gemm_mma_fp16acc_m16n32_k32_b16_vs: currently requires "
    ...
    gemm_mma_fp16acc_m16n32_k32_b16_vs_kernel<<<grid, block, 0, stream>>>(
        dA, dB, dC, M, N, K);
