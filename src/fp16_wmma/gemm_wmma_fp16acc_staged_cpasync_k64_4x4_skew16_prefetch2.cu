...
__global__ void gemm_wmma_fp16acc_staged_cpasync_k64_4x4_skew16_prefetch2_kernel(
    const half* __restrict__ A,
    const half* __restrict__ B,
    ...
    for (int k0 = 0; k0 < K; k0 += BLOCK_K) {
        ...
        wmma::fragment<wmma::matrix_a, WMMA_M, WMMA_N, WMMA_K, half, wmma::row_major> a_frag0;
        ...
        // preload kk = 0
        {
            ...
            wmma::load_matrix_sync(a_frag0, A_smem_ptr, SMEM_STRIDE_A);
            wmma::load_matrix_sync(b_frag0, B_smem_ptr, SMEM_STRIDE_B);
        }
        
        // preload kk = 16
        {
            const half* A_smem_ptr = &smemA[read_buf][(warp_m * WMMA_M) * SMEM_STRIDE_A + 16];
            const half* B_smem_ptr = &smemB[read_buf][16 * SMEM_STRIDE_B + warp_n * WMMA_N];
            wmma::load_matrix_sync(a_frag1, A_smem_ptr, SMEM_STRIDE_A);
            wmma::load_matrix_sync(b_frag1, B_smem_ptr, SMEM_STRIDE_B);
        }
        
        // mma kk = 0
        wmma::mma_sync(c_frag, a_frag0, b_frag0, c_frag);
        
        // preload kk = 32 into frag0
        {
            const half* A_smem_ptr = &smemA[read_buf][(warp_m * WMMA_M) * SMEM_STRIDE_A + 32];
            const half* B_smem_ptr = &smemB[read_buf][32 * SMEM_STRIDE_B + warp_n * WMMA_N];
            wmma::load_matrix_sync(a_frag0, A_smem_ptr, SMEM_STRIDE_A);
            wmma::load_matrix_sync(b_frag0, B_smem_ptr, SMEM_STRIDE_B);
        }
        ...
        // preload kk = 48 into frag1
        {
            const half* A_smem_ptr = &smemA[read_buf][(warp_m * WMMA_M) * SMEM_STRIDE_A + 48];
            const half* B_smem_ptr = &smemB[read_buf][48 * SMEM_STRIDE_B + warp_n * WMMA_N];
            wmma::load_matrix_sync(a_frag1, A_smem_ptr, SMEM_STRIDE_A);
            wmma::load_matrix_sync(b_frag1, B_smem_ptr, SMEM_STRIDE_B);
        }
...
void launch_gemm_wmma_fp16acc_staged_cpasync_k64_4x4_skew16_prefetch2(
    const half* dA, const half* dB, float* dC,
    int M, int N, int K, cudaStream_t stream) {

    if (M % 16 != 0 || N % 16 != 0 || K % 64 != 0) {
        std::fprintf(stderr,
            "gemm_wmma_fp16acc_staged_cpasync_k64_4x4_skew16_prefetch2: currently requires M,N to be multiples of 16 and K to be a multiple of 64. "
            "Got M=%d N=%d K=%d\n",
            M, N, K);
    ...
    gemm_wmma_fp16acc_staged_cpasync_k64_4x4_skew16_prefetch2_kernel<<<grid, block, 0, stream>>>(dA, dB, dC, M, N, K);
