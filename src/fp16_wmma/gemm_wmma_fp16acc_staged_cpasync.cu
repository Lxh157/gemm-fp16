// src/gemm_wmma_fp16acc_staged_cpasync.cu
#include "utils.cuh"
...
constexpr int A_CHUNK_BYTES = 16;
constexpr int B_CHUNK_BYTES = 16;
constexpr int A_CHUNK_HALF  = A_CHUNK_BYTES / sizeof(half); // 8 half
constexpr int B_CHUNK_HALF  = B_CHUNK_BYTES / sizeof(half); // 8 half

constexpr int A_ROW_CHUNKS = BLOCK_K / A_CHUNK_HALF; // 16 / 8 = 2
constexpr int B_ROW_CHUNKS = BLOCK_N / B_CHUNK_HALF; // 64 / 8 = 8

constexpr int A_NUM_CHUNKS = BLOCK_M * A_ROW_CHUNKS; // 32 * 2 = 64
constexpr int B_NUM_CHUNKS = BLOCK_K * B_ROW_CHUNKS; // 16 * 8 = 128
...
__device__ __forceinline__ void load_stage_to_shared_cpasync(
    ...
    // A tile: [32, 16], two 16B chunks per row
    if (tid < A_NUM_CHUNKS) {
        const int row = tid / A_ROW_CHUNKS;            // 0..31
        const int chunk_in_row = tid % A_ROW_CHUNKS;   // 0..1
        const int col = chunk_in_row * A_CHUNK_HALF;   // 0 or 8
    ...
    // B tile: [16, 64], eight 16B chunks per row
    if (tid < B_NUM_CHUNKS) {
        const int row = tid / B_ROW_CHUNKS;            // 0..15
        const int chunk_in_row = tid % B_ROW_CHUNKS;   // 0..7
        const int col = chunk_in_row * B_CHUNK_HALF;   // 0,8,16,...,56
...
__global__ void gemm_wmma_fp16acc_staged_cpasync_kernel(
    const half* __restrict__ A,
    const half* __restrict__ B,
    ...
    for (int k0 = 0; k0 < K; k0 += BLOCK_K) {
        ...
        // issue async copy for next stage as early as possible
        if (next_k0 < K) {
            load_stage_to_shared_cpasync(
                A, B,
...
void launch_gemm_wmma_fp16acc_staged_cpasync(
    const half* dA, const half* dB, float* dC,
    int M, int N, int K, cudaStream_t stream) {

    if (M % 16 != 0 || N % 16 != 0 || K % 16 != 0) {
        std::fprintf(stderr,
            "gemm_wmma_fp16acc_staged_cpasync: currently requires M,N,K to be multiples of 16. "
            "Got M=%d N=%d K=%d\n",
            M, N, K);
    ...
    gemm_wmma_fp16acc_staged_cpasync_kernel<<<grid, block, 0, stream>>>(dA, dB, dC, M, N, K);
