// src/gemm_mma_fp16acc_m16n32_k32_4x2.cu
#include "utils.cuh"
...
__global__ void gemm_mma_fp16acc_m16n32_k32_4x2_kernel(
    ...
    __shared__ half smemB[BLOCK_K * SMEM_STRIDE_B]; // [32][72]
...
void launch_gemm_mma_fp16acc_m16n32_k32_4x2(const half* dA, const half* dB,
                                            float* dC, int M, int N, int K,
                                            cudaStream_t stream) {
    if (K % BLOCK_K != 0) {
        std::fprintf(stderr,
                     "gemm_mma_fp16acc_m16n32_k32_4x2: currently requires K %% 32 == 0. "
                     "Got K=%d\n",
                     K);
    ...
    gemm_mma_fp16acc_m16n32_k32_4x2_kernel<<<grid, block, 0, stream>>>(
        dA, dB, dC, M, N, K);
