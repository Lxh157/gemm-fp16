// src/gemm_mma_fp16acc_m16n32_k32.cu
#include "utils.cuh"
...
// Each warp computes one 16x32 output tile using four m16n8k16 MMA ops.
// This variant stages K=32, so each shared-memory load feeds two MMA-K steps.
...
constexpr int WARP_TILE_N = 4 * MMA_N;
constexpr int BLOCK_M = WARPS_PER_BLOCK_M * WARP_TILE_M; // 64
constexpr int BLOCK_N = WARPS_PER_BLOCK_N * WARP_TILE_N; // 32
...
void launch_gemm_mma_fp16acc_m16n32_k32(const half* dA, const half* dB, float* dC,
    ...
    if (K % BLOCK_K != 0) {
        std::fprintf(stderr,
                     "gemm_mma_fp16acc_m16n32_k32: currently requires K %% 32 == 0. "
                     "Got K=%d\n",
                     K);
    ...
    gemm_mma_fp16acc_m16n32_k32_kernel<<<grid, block, 0, stream>>>(dA, dB, dC, M, N, K);
